 ...
 * 后两者都以**包 id** 为键，因此 `require('dsh-header-balance/balance')` 这类
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const STYLE_ID = 'dsh-header-balance-css'
...
function describeBalance(result) {
  ...
  return {
    ...
    function installStyle() {
      ...
      style.id = STYLE_ID
      style.textContent = [
        '.dsh-header-balance { display:inline-flex; align-items:center; gap:7px;',
        ...
        '.dsh-header-balance-value { font-weight:400; font-variant-numeric:tabular-nums; }',
        '.dsh-header-balance-refresh { display:inline-flex; align-items:center;',
        ...
        '.dsh-header-balance-refresh:hover { opacity:0.9; }',
        // 手机端头部行被汉堡按钮占去 56px，余额芯片省略「余额」二字。
        `@media ${NARROW_QUERY} { .dsh-header-balance-label { display:none; } }`,
      ].join('\n')
    ...
    function BalanceChip({ connection }) {
      ...
      return react.createElement(
        ...
        react.createElement('span', { className: 'dsh-header-balance-label' }, '余额'),
        react.createElement('span', { className: 'dsh-header-balance-value' }, value),
        react.createElement(
          'button',
          {
            type: 'button',
            className: 'dsh-header-balance-refresh',
            title: '刷新余额',
            'aria-label': '刷新余额',
            onClick: (event) => {
    ...
    return { apply, inject, name: 'dsh-header-balance' }
  },
})
